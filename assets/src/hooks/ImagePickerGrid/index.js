export default app => ({
  mounted() {
    ...
    // Intercept shift+clicks on image rows BEFORE LiveView's click handler.
    ...
    this._clickHandler = (ev) => {
      ...
      if (this.targetComponent) {
        this.pushEventTo(this.targetComponent, 'organize_select_image', payload)
      } else {
        this.pushEvent('organize_select_image', payload)
      }
    }
    ...
    this._dragStartHandler = (ev) => {
      ...
      ghost.className = 'image-picker-drag-ghost'
    ...
    this.handleEvent('image_picker_selection_changed', ({ selected_ids, organize_ids }) => {
      ...
      this.el.querySelectorAll('.image-picker__image[data-id]').forEach(el => {
