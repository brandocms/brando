...
then
  ...
  cd assets/frontend && yarn && yarn upgrade @brandocms/jupiter @brandocms/europacss && cd ../backend && yalc add brandojs && yarn && yarn lint --fix && cd ../../
  direnv allow
  mix do deps.get, deps.compile --force
  mix brando.upgrade
  mix ecto.setup
fi
