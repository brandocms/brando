defmodule BrandoAdmin.UserSettingsController do
    ...
    case Users.apply_user_email(user, password, user_params) do
      {:ok, applied_user} ->
        Users.deliver_update_email_instructions(
          applied_user,
          user.email,
          &Brando.helpers().admin_user_settings_url(conn, :confirm_email, &1)
        )
        ...
        |> redirect(to: Brando.helpers().admin_user_settings_path(conn, :edit))
  ...
  def update(conn, %{"action" => "update_password"} = params) do
    ...
    case Users.update_user_password(user, password, user_params) do
      {:ok, user} ->
        ...
        |> put_session(:user_return_to, Brando.helpers().admin_user_settings_path(conn, :edit))
