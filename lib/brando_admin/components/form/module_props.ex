    ...
    <div class="properties shaded">
      <div class="inner">
        ...
        <div class="button-group">
          <button :on-click={@show_modal} phx-value-id={"#{@form.id}-#{@key}-icon"} class="secondary" type="button">Edit icon</button>
        </div>
        ...
        <div class="refs">
          <h2>
            ...
            <button :on-click={@show_modal} phx-value-id={"#{@form.id}-#{@key}-create-ref"} type="button" class="circle">
          ...
          <Modal title="Create ref" id={"#{@form.id}-#{@key}-create-ref"} narrow>
            ...
            {#if @ref_name}
              <div class="button-group">
                <button type="button" :on-click={@create_ref} phx-value-type={"text"} phx-value-id={"#{@form.id}-#{@key}-create-ref"} class="secondary">
                  Text
                </button>
                <button type="button" :on-click={@create_ref} phx-value-type={"header"} phx-value-id={"#{@form.id}-#{@key}-create-ref"} class="secondary">
                  Header
                </button>
                <button type="button" :on-click={@create_ref} phx-value-type={"picture"} phx-value-id={"#{@form.id}-#{@key}-create-ref"} class="secondary">
                  Picture
                </button>
              </div>
            {/if}
            ...
            <Inputs form={@form} for={:refs} :let={form: ref, index: idx}>
              <li class="padded">
                {#for ref_data <- inputs_for_block(ref, :data)}
                  ...
                  <div class="actions">
                    <button class="tiny" type="button" :on-click={@show_modal} phx-value-id={"#{@form.id}-#{@key}-ref-#{idx}"}>
                        ...
                        {#case input_value(ref_data, :type)}
                          ...
                          {#match "svg"}
                            {#for block_data <- inputs_for_block(ref_data, :data)}
                              ...
                              <Input.Code id={"#{@form.id}-ref-#{@key}-#{input_value(ref, :name)}-svg-code"} form={block_data} field={:code} />
                            {/for}
        ...
        <Modal title="Create var" id={"#{@form.id}-#{@key}-create-var"} narrow>
          ...
          <div class="button-group">
            <button disabled={!@var_name} type="button" :on-click={@create_var} phx-value-type={"text"} phx-value-id={"#{@form.id}-#{@key}-create-var"} class="secondary">
              Text
            </button>
            <button disabled={!@var_name} type="button" :on-click={@create_var} phx-value-type={"string"} phx-value-id={"#{@form.id}-#{@key}-create-var"} class="secondary">
              String
            </button>
            <button disabled={!@var_name} type="button" :on-click={@create_var} phx-value-type={"html"} phx-value-id={"#{@form.id}-#{@key}-create-var"} class="secondary">
              Html
            </button>
            <button disabled={!@var_name} type="button" :on-click={@create_var} phx-value-type={"datetime"} phx-value-id={"#{@form.id}-#{@key}-create-var"} class="secondary">
              Datetime
            </button>
            <button disabled={!@var_name} type="button" :on-click={@create_var} phx-value-type={"boolean"} phx-value-id={"#{@form.id}-#{@key}-create-var"} class="secondary">
              Boolean
            </button>
            <button disabled={!@var_name} type="button" :on-click={@create_var} phx-value-type={"color"} phx-value-id={"#{@form.id}-#{@key}-create-var"} class="secondary">
              Color
            </button>
          </div>
        </Modal>
        ...
        <div class="vars">
          <h2>
            ...
            <button :on-click={@show_modal} phx-value-id={"#{@form.id}-#{@key}-create-var"} type="button" class="circle">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18"><path fill="none" d="M0 0h24v24H0z"/><path d="M11 11V5h2v6h6v2h-6v6h-2v-6H5v-2z"/></svg>
            </button>
          </h2>
          <ul
            ...
            <PolyInputs form={@form} for={:vars} :let={form: var, index: idx}>
              <li class="var padded sort-handle draggable" data-id={idx}>
                ...
                <div class="actions">
                  <button class="tiny" type="button" :on-click={@show_modal} phx-value-id={"#{@form.id}-#{@key}-var-#{idx}"}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="12" height="12"><path fill="none" d="M0 0h24v24H0z"/><path d="M6.414 16L16.556 5.858l-1.414-1.414L5 14.586V16h1.414zm.829 2H3v-4.243L14.435 2.322a1 1 0 0 1 1.414 0l2.829 2.829a1 1 0 0 1 0 1.414L7.243 18zM3 20h18v2H3v-2z"/></svg>
                  </button>
