  ...
  def render(assigns) do
    ...
    <div
      ...
      <.live_component module={Modal} title="Configure" id={"#{@uid}_config"} wide={@wide_config}>
        ...
        <% end %>
        <:footer>
          <button type="button" class="primary" phx-click={JS.push("hide_config_modal", target: @myself)} phx-value-id={"#{@uid}_config"}>
            <%= gettext "Close" %>
          </button>
          <%= if @config_footer do %>
      ...
      <div
        ...
        <div class="block-actions" id={"#{@uid}-block-actions"}>
          ...
          <button
            ...
            phx-click={JS.push("show_config_modal", target: @myself)}>
            <%= if @type == "module" do %>
