defmodule Brando.Type.Image.Config do
  @moduledoc """
  ...
  def cast(val) when is_binary(val) do
    val = Poison.decode!(val, as: Brando.Type.Image.Config, keys: :atoms!)
    {:ok, val}
  end
  ...
  """
  def blank?(_), do: %Brando.Type.Image.Config{}
  ...
  def load(val) do
    val = Poison.decode!(val, as: Brando.Type.Image.Config, keys: :atoms!)
    if val == nil, do: val = %Brando.Type.Image.Config{}
    {:ok, val}
  end
