defmodule Brando.Villain.TemplateAdapter.Heex do
  ...
  def render_child_module(child_module, child_block, vars, refs, forloop, _parent_module_id, opts) do
    ...
    HeexRenderer.render_to_string(
      "child_#{child_module.id}_#{child_block.uid}",
      child_module.code,
      assigns
    )
  end
  ...
  defp build_assigns(module, block, processed_vars, processed_refs, opts) do
    ...
    # Add system assigns from Liquex context if available
    base =
      if context = opts[:context] do
        base
        |> maybe_put_from_context(context, "identity", :identity)
        |> maybe_put_from_context(context, "globals", :globals)
        |> maybe_put_from_context(context, "navigation", :navigation)
        |> maybe_put_from_context(context, "language", :language)
        |> maybe_put_from_context(context, "locale", :locale)
        |> maybe_put_from_context(context, "entry", :entry)
        |> maybe_put_from_context(context, "links", :links)
        |> maybe_put_from_context(context, "url", :url)
        |> maybe_put_from_context(context, "request", :request)
      else
        base
      end

    # Merge vars at the top level (e.g., @my_var)
    Enum.reduce(processed_vars, base, fn {k, v}, acc ->
  ...
  defp maybe_put_from_context(assigns, context, context_key, assigns_key) do
    case Liquex.Context.get(context, context_key) do
      nil -> assigns
      value -> Map.put(assigns, assigns_key, value)
    end
  end
