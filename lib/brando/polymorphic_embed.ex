defmodule Brando.PolymorphicEmbed do
  @moduledoc """
  Modified version of PolymorphicEmbed
  ...
  defp get_field_options(schema, field) do
    try do
      ...
      {:parameterized, Brando.PolymorphicEmbed, options} ->
        Map.put(options, :array?, false)

      {:array, {:parameterized, Brando.PolymorphicEmbed, options}} ->
        Map.put(options, :array?, true)

      {_, {:parameterized, Brando.PolymorphicEmbed, options}} ->
        Map.put(options, :array?, false)
  ...
  defp merge_polymorphic_keys(map, changes, types, msg_func) do
    Enum.reduce(types, map, fn
      {field, {:parameterized, Brando.PolymorphicEmbed, _opts}}, acc ->
      ...
      {field, {:array, {:parameterized, Brando.PolymorphicEmbed, _opts}}}, acc ->
