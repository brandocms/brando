...
- [x] **S-2 — pin `phoenix_live_view 1.2.8` next to both `client_proxy.ex:542-545`
      citations** — line numbers re-verified against the vendored 1.2.8: `put_view/3` monitors at `:848`, registers `state.pids` at `:856`, children reach it at `:1001`, `fetch_view_by_pid/2` at `:908-912`. (The plan said `:849`; the monitor is on `:848`.) (`live_case.ex:99, 141`), as the `view.ts` mirror elsewhere in
