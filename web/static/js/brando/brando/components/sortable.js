...
function Sortable(el, options) {
  ...
  _customEvents.forEach(function(name) {
    ...
    _on(el, name.substr(2)
      .toLowerCase(), options[name]);
  }, this);
...
Sortable.prototype = /** @lends Sortable.prototype */ {
  ...
  _onTouchMove: function( /**TouchEvent*/ evt) {
    if (tapEvt) {
      var touch = evt.touches ? evt.touches[0] : evt,
        ...
        translate3d = evt.touches ? 'translate3d(' + dx + 'px,' + dy + 'px,0)' :
  ...
  _onDragOver: function( /**Event*/ evt) {
    ...
    if (activeGroup && !options.disabled &&
      ...
      if ((el.children.length === 0) || (el.children[0] === ghostEl) ||
      ...
      } else if (target && !target.animated && target !== dragEl && (target.parentNode[
          expando] !== void 0)) {
        if (lastEl !== target) {
          lastEl = target;
          lastCSS = _css(target);
        ...
        var targetRect = target.getBoundingClientRect(),
          ...
          halfway = (floating ? (evt.clientX - targetRect.left) / width : (evt.clientY -
            targetRect.top) / height) > 0.5,
  ...
  _onDrop: function( /**Event*/ evt) {
    ...
    if (evt) {
      ...
      if (dragEl) {
        ...
        // Drag end event
        Sortable.active && _dispatchEvent(rootEl, 'end', dragEl, rootEl, oldIndex,
          newIndex);
      }
