...
class Stats {
    static setup() {
        ...
        this.metrics = [
            {key: "total", chart: totalMemoryChart, values: []},
            {key: "system", chart: systemMemoryChart, values: []},
            {key: "atom", chart: atomMemoryChart, values: []},
            {key: "binary", chart: binaryMemoryChart, values: []},
            {key: "ets", chart: etsMemoryChart, values: []},
            {key: "process", chart: processMemoryChart, values: []},
            {key: "code", chart: codeMemoryChart, values: []}
        ];
    ...
    static updateCharts(payload) {
        for (let x = 0; x < this.metrics.length; x++) {
            ...
            $(`#${metric.key}-memory .text`).html(this.humanFileSize(parseInt(payloadValue), false));
    ...
    static updateUptime(payload) {
        if (payload.status.uptime) {
            $('#system-uptime .display').html(payload.status.uptime);
        }
    }
