defmodule Brando.Images.UtilsTest do
  ...
  @cfg %{allowed_mimetypes: ["image/jpeg", "image/png"],
         ...
         sizes: %{"small" =>  %{"size" => "300", "quality" => 100},
                  ...
                  "thumb" =>  %{"size" => "150x150", "quality" => 100,
                                "crop" => true},
                  "micro" =>  %{"size" => "25x25", "quality" => 100,
                                "crop" => true}}}

  @broken_cfg Map.put(@cfg, :upload_path,
                      Path.join(["images", "default", "sample.png"]))
  @random_filename_cfg Map.put(@cfg, :random_filename, true)
  @upload %Plug.Upload{content_type: "image/png",
                       filename: "sample.png",
                       path: "#{Path.expand("../../", __DIR__)}" <>
                             "/fixtures/sample.png"}
  @blank_upload %Plug.Upload{content_type: "image/png",
                             filename: "",
                             path: "#{Path.expand("../../", __DIR__)}" <>
                                   "/fixtures/sample.png"}
  @slug_upload %Plug.Upload{content_type: "image/png",
                            filename: "file with spaces.png",
                            path: "#{Path.expand("../../", __DIR__)}" <>
                                   "/fixtures/sample.png"}
  @image %Brando.Type.Image{credits: nil, optimized: false,
             path: "images/default/sample.png",
             sizes: %{"large" => "images/default/large/sample.png",
  ...
  test "do_upload/2" do
    assert Upload.do_upload(@upload, @cfg)
           == {:ok, Map.put(@image, :optimized, true)}
    refute Upload.do_upload(@upload, @cfg)
           == {:ok, @image}
    ...
    assert_raise Brando.Exception.UploadError,
                 fn -> Upload.do_upload(@slug_upload, @broken_cfg) end
    assert_raise Brando.Exception.UploadError,
                 fn -> Upload.do_upload(@blank_upload, @cfg) end
  end
