defmodule Brando.Repo.Migrations.Brando180TranslateImageTexts do
  ...
  @moduledoc """
  An image's alt text, title and credits become one map per field, keyed by
  content language. Existing text moves under the default language; empty
  text becomes NULL.

  Code that read `image.alt`, `image.title` or `image.credits` as a string
  now gets a map: use `Brando.Images.text(image, :alt, language)`.
  """
