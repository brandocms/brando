...
test('create a simple text module', async ({ page }) => {
  ...
  await refModal.getByRole('textbox', { name: 'Default text and preview', exact: true }).fill('Tekst')
  ...
  expect(configured).toContain('orderedList')
  expect(configured).not.toContain('blockquote')
  await expect(refModal.getByRole('textbox', { name: 'Default text and preview', exact: true })).toHaveText('Tekst')
...
test('create, edit, duplicate, persist and delete refs and vars', async ({ page }) => {
  ...
  await refModal.getByRole('textbox', { name: 'Default text and preview', exact: true }).fill('Intro text')
  await refModal.getByRole('button', { name: 'Done' }).click()
  ...
  await expect(refModal.getByRole('textbox', { name: 'Default text and preview', exact: true })).toHaveText('Intro text')
  await refModal.getByRole('button', { name: 'Done' }).click()
