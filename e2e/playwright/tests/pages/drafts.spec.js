...
test.describe('Entry recovery copies', () => {
  ...
  test('browses a recovery table and keeps inspected content open across autosave patches', async ({ page }, testInfo) => {
    ...
    const showUnchanged = preview.getByRole('checkbox', { name: 'Show unchanged content', exact: true })
    ...
    await expect(showUnchanged).not.toBeChecked()
    await showUnchanged.focus()
    await showUnchanged.press('Space')
    ...
    await expect(showUnchanged).toBeChecked()
    await expect(preview.locator('.text-diff-line.is-eq:visible').first()).toBeVisible()
    await expect(preview).not.toContainText('Editing behind the recovery panel')
    ...
    await expect(inspector).not.toHaveAttribute('open', '')
    await showUnchanged.uncheck()
    await expect(preview.locator('.text-diff-line.is-eq:not(.is-change-heading):visible')).toHaveCount(0)
  ...
  test('recovers a new entry and unsaved block after reload, then resolves the copy on save', async ({ page }, testInfo) => {
    ...
    await expect(panel.locator('.draft-content-preview')).toBeVisible()
    ...
    await expect(page.getByLabel('Title', { exact: true })).toHaveValue('Autumn campaign')
    await page.getByRole('button', { name: 'Review recovery copy', exact: true }).click()
    await expect(page.locator('.draft-content-preview')).toContainText('Autumn campaign revised')
    const diff = page.locator('.draft-content-preview .admin-text-diff').filter({ hasText: 'Block 1' })
    const preview = page.locator('.draft-content-preview')
    const showUnchanged = preview.getByRole('checkbox', { name: 'Show unchanged content', exact: true })
    await expect(showUnchanged).not.toBeChecked()
    await expect(diff.locator('.text-diff-line.is-eq:not(.is-change-heading):visible')).toHaveCount(0)
    await expect(diff.locator('.is-change-heading')).toBeVisible()
    await expect(diff.locator('del')).toContainText('A new season of ideas')
    await expect(diff.locator('ins')).toContainText('An updated season of ideas')
    await expect(diff).not.toContainText('Extensions')
    await page.context().grantPermissions(['clipboard-read', 'clipboard-write'])
    await page.getByRole('button', { name: 'Copy text', exact: true }).click()
    const copied = await page.evaluate(() => navigator.clipboard.readText())
    expect(copied).toContain('An updated season of ideas')
    expect(copied).not.toContain('A new season of ideas')
    expect(copied).toContain('Alignment')
    await preview.evaluate(el => el.scrollIntoView({ block: 'start' }))
    await preview.screenshot({ path: testInfo.outputPath('recovery-changes-only-desktop.png') })
    await showUnchanged.check()
    await expect(diff.locator('.text-diff-line.is-eq').filter({ hasText: 'Alignment' })).toBeVisible()
    await preview.screenshot({ path: testInfo.outputPath('recovery-all-content-desktop.png') })
    await showUnchanged.uncheck()
    await page.setViewportSize({ width: 390, height: 1000 })
    await expect.poll(() => diff.evaluate(el => el.scrollWidth <= el.clientWidth)).toBe(true)
    await preview.evaluate(el => el.scrollIntoView({ block: 'start' }))
    await preview.screenshot({ path: testInfo.outputPath('recovery-changes-only-mobile.png') })
    await page.setViewportSize({ width: 1440, height: 1100 })
    await page.getByRole('button', { name: 'Restore recovery copy', exact: true }).click()
    ...
    await expect(page.getByLabel('Title', { exact: true })).toHaveValue('Autumn campaign revised')
    await expect(page.locator('.header-block textarea')).toHaveValue('An updated season of ideas')
    // A later title-only edit must hide the wholly unchanged block.
    await page.getByLabel('Title', { exact: true }).fill('Only the title changed')
    await expect(page.getByTestId('draft-status')).toContainText('Recovery copy saved at', { timeout: 25000 })
    await page.reload()
    await page.getByRole('button', { name: 'Review recovery copy', exact: true }).click()
