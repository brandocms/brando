...
test('opens image editor, adjusts focal point, and saves', async ({ page }, testInfo) => {
  ...
  // A language is marked missing only once another one has text.
  await expect(altField.locator('.i18n-tab[data-missing="true"]')).toHaveCount(0)
  await altInput.fill('Library description')
  await syncLV(page)
  await expect(altTab).toHaveAttribute('data-missing', 'false')
  await expect(altField.locator('.i18n-tab[data-missing="true"]')).not.toHaveCount(0)
