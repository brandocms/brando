...
test('notes are opt-in, keep their subtree on save, and renumber existing markers on load', async ({ page }, testInfo) => {
  ...
  await page.getByRole('link', { name: 'Footnote persistence', exact: true }).click()
  await syncLV(page)
  ...
  await page.getByRole('button', { name: 'Save (⌘S)', exact: true }).click()
  await expect(page).toHaveURL(/\/admin\/pages$/)
  await page.getByRole('link', { name: 'Footnote persistence edited', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Edit footnote 1', exact: true })).toBeVisible()
  await expect(text.locator('p')).toHaveText(`${originalText}1`)
...
test('a note uses the existing image, video and file controls and keeps media on reload', async ({ page }) => {
  ...
  await expect(drawer.locator('.picture-block .media-field--block:visible img')).toBeVisible({ timeout: 30000 })
  await add('Note video')
  await drawer.locator('.video-block .media-field:visible').getByRole('button', { name: 'Browse library', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Save (⌘S)', exact: true }).click()
  await expect(page).toHaveURL(/\/admin\/pages$/)
  await page.getByRole('link', { name: 'Footnote media', exact: true }).click()
  await page.getByRole('button', { name: 'Edit footnote 1', exact: true }).click()
  await expect(drawer.locator('.picture-block .media-field--block:visible img')).toBeVisible()
  await expect(drawer.locator('.video-block .media-field:visible')).toContainText('Footnote film')
  await expect(fileVar).toContainText('footnote-source.pdf')
})

test('a recovery copy restores an unsaved note and keeps it through the next save', async ({ page }) => {
  test.setTimeout(90000)
  await page.goto('/admin/pages/create')
  await syncLV(page)
  await page.getByLabel('Title', { exact: true }).fill('Recovered footnote')
  await page.getByLabel('URI', { exact: true }).fill('recovered-footnote')
  await appendModule(page, 'Article with notes')
  const editor = page.locator('.entry-block').last().locator('.tiptap[contenteditable=true]').first()
  const originalText = await placeCaretAtEnd(editor)
  await page.getByRole('button', { name: 'Add footnote' }).click()
  const drawer = page.locator('.block-slot-drawer.visible')
  await drawer.locator('.tiptap[contenteditable=true]').fill('A source recovered with its reference.')
  await drawer.getByRole('button', { name: 'Done', exact: true }).click()
  ...
  await expect(editor.locator('p')).toHaveText(`${originalText}1`)
  await page.getByRole('button', { name: 'Edit footnote 1', exact: true }).click()
  await expect(drawer.locator('.tiptap[contenteditable=true]')).toHaveText('A source recovered with its reference.')
  await drawer.getByRole('button', { name: 'Done', exact: true }).click()
  await page.getByRole('button', { name: 'Save (⌘S)', exact: true }).click()
  await expect(page).toHaveURL(/\/admin\/pages$/)
  await page.getByRole('link', { name: 'Recovered footnote', exact: true }).click()
  await expect(editor.locator('p')).toHaveText(`${originalText}1`)
})
