...
test('Markdown folder selection adds multiple sources and is translated', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.request.post('/e2e/setup_fixtures/markdown-source')
  await norwegian(page)
  await page.goto('/admin/config/markdown-sources')
  await syncLV(page)
  await expect(page.getByRole('heading', { name: 'Markdown-kilder', exact: true })).toBeVisible()
  ...
  await connection.blur()
  await screenshot(page, testInfo, 'markdown-desktop')
  await page.getByRole('button', { name: 'Fra en mappe', exact: true }).click()
  await page.getByLabel('Mappe', { exact: true }).fill('guides')
  await page.getByRole('button', { name: 'Finn Markdown-filer', exact: true }).click()
  await expect(page.locator('#markdown-folder-selection input')).toHaveCount(3)
  await screenshot(page, testInfo, 'markdown-folder-desktop')
  await narrow(page)
  await screenshot(page, testInfo, 'markdown-folder-mobile')
  await page.getByRole('button', { name: 'Legg til 3 dokumenter', exact: true }).click()
  await expect(page.locator('.markdown-source-notice')).toContainText('2 dokumenter er lagt til')
  await expect(page.locator('.markdown-source-row')).toHaveCount(3)
})

test('global sets have compact rows and readable variable help at both widths', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.goto('/admin/config/global_sets/create')
  await syncLV(page)
  await page.getByLabel('Label', { exact: true }).fill('Site configuration')
  await page.getByLabel('Key', { exact: true }).fill('design_config')
  await page.getByRole('button', { name: 'Add entry', exact: true }).click()
  const disclosure = page.locator('#global_set_vars_0-edit .variable-header')
  ...
  expect(help.y - (control.y + control.height)).toBeGreaterThanOrEqual(6)
  await screenshot(page, testInfo, 'globals-editor-desktop')
  await narrow(page)
  await screenshot(page, testInfo, 'globals-editor-mobile')
  ...
  await page.setViewportSize({ width: 1440, height: 1000 })
  await syncLV(page)
  await screenshot(page, testInfo, 'globals-list-desktop')
  await narrow(page)
  await screenshot(page, testInfo, 'globals-list-mobile')
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.getByRole('link', { name: /Site configuration/ }).click()
  await page.locator('#global_set_vars_0-edit .variable-header').click()
  await screenshot(page, testInfo, 'globals-editor-norwegian')
  await narrow(page)
  await screenshot(page, testInfo, 'globals-editor-norwegian-mobile')
})

test('galleries share the resource listing design', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.request.post('/e2e/setup_fixtures/gallery-preview')
  await norwegian(page)
  ...
  await narrow(page)
  const firstRow = page.locator('.content-list .list-row').first()
  const cover = await firstRow.locator('.cover').boundingBox()
  const title = await firstRow.locator('.main-content > div:has(> .entry-link)').boundingBox()
  expect(title.x - (cover.x + cover.width)).toBeGreaterThanOrEqual(8)
  expect(title.width).toBeGreaterThan(180)
  await screenshot(page, testInfo, 'galleries-mobile')
})
