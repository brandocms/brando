...
The media implementation commit `c1d1af6d5` merged cleanly with `next` at `2d808c8d4`, including its newer recovery-copy behavior and blueprint workspace styling. The combined tree passed the E2E consumer asset build, all 15 JavaScript recovery/upload-progress checks, and all 36 browser checks in a fresh targeted run (4.4 minutes):

- `tests/admin-workspaces.spec.js` — 13 desktop/mobile workspace checks.
- `tests/blocks/block-media-fields.spec.js` — 7 media interaction checks.
- `tests/pages/draft-media.spec.js` — 9 media recovery and storage checks.
- `tests/pages/drafts.spec.js` — 7 entry/block recovery checks.

The integration run used `./test_e2e.sh --reset` and completed strict compilation, database reset, migration rollback/forward validation, and seeding. No merge fixes were required.
